module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const rows = await Promise.all(
      assets.map(async (a) => {
        ...
        try {
          ...
          console.error(`L2 fetch failed for ${a.hlCoin}`, err);
        }
    ...
    return res.status(200).json({ updatedAt: new Date().toISOString(), fxRate, rows });
  } catch (err) {
    console.error('market api failed', err);
    return res.status(500).json({ error: err.message || 'market fetch failed' });
  }
};
...
async function fetchKrxSnapshot(assets) {
  ...
  if (!res.ok) throw new Error(`KRX API failed: ${res.status}`);
  const data = await res.json();
  const map = new Map();
...
async function fetchL2Book(coin) {
  ...
  if (!res.ok) throw new Error(`HL l2 failed: ${res.status}`);
  const data = await res.json();
  return {
